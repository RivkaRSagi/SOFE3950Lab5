...
void *customer_thread(void *arg) {
  ...
  while (true) {
    ...
    if (request_resources(custNum, request) == 0) {
      // If resources are successfully allocated, simulate work (e.g.,
      // sleep for 1 second)
      // Print the successful request status, protected by the print mutex
      pthread_mutex_lock(&print_mutex);
      printf("Customer %d request succeeded!\n", custNum);
      pthread_mutex_unlock(&print_mutex);
      sleep(1);
